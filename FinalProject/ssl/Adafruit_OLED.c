...
*  SPI.
*/
    unsigned long cDummy;

    GPIOPinWrite(DC_BASE,DC_PIN,LOW); //Say this is a command

    GPIOPinWrite(OLEDCS_BASE,OLEDCS_PIN,LOW); // active low

    MAP_SPICSEnable(GSPI_BASE);     // Enable chip select

    MAP_SPIDataPut(GSPI_BASE,c);  // Push character over SPI

    MAP_SPIDataGet(GSPI_BASE,&cDummy);       // clean up receive register

    GPIOPinWrite(OLEDCS_BASE,OLEDCS_PIN,HIGH); // turn off OLED again

    MAP_SPICSDisable(GSPI_BASE);       // disable for safety
...
*/
    ...
    GPIOPinWrite(DC_BASE,DC_PIN,HIGH); //Say this is data
